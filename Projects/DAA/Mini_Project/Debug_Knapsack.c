...
int main(int argc, char **agrv){
    ...
    switch (choice){
        case 0:
            printw("Enter the number of item: ");
            scanf("%d", &n);
            printw("Enter the Maximum Weight: ");
            scanf("%d", &Max_W);
            printw("Enter the Profits:\n");
            Profits = (int *)malloc(n*sizeof(int));
            Profits = custom_assignment(n, "Profit");
            printw("Enter the Weights:\n");
    ...
    printw("Max Weight -%d\nMax Profit - %d\nNumber of Items - %d\n", Max_W, Max_P, n);
    printw("Enter the Maximum Profit: ");
    scanf("%d", &Input_MaxP);
    if(Max_P != Input_MaxP){
...
int * custom_assignment(int n, char* Str){
    int *arr = (int *)malloc(n*sizeof(int));
    for(int i = 0; i < n; ++i){
        printw("Enter the %s of item %d: ", Str, i+1);
        scanf("%d", &arr[i]);
    }
    return arr;
}
...
void Profitable_Items(int Profits[], int Weights[],int n, int Max_W){
    ...
    printw("Before Sorting - \n");
    printw("Weight - %d\n", Max_W);
    printw("Number of Items = %d\n", n);
    printw("Item_nos\tProfit\tWeight\tProfit/Weight\n");
    for(int i = 0; i < n; ++i){
    ...
    // printw("Weight - %d\n", Max_W);
    // printw("Number of Items = %d\n", n);
    // printw("Item nos\tProfit\tWeight\tProfit/Weight\t\n");
    // ptr = &item_nos[0];
    // for(int i = 0; i < n; ++i){
    ...
    printw("Max Profit in Profitable_Item - %d\n", max_P_);
}
